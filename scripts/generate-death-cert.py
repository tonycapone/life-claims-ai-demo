"""
Generate a realistic synthetic death certificate PDF for the ClaimPath demo.
Subject: John Michael Smith (policy LT-29471)

Usage:  python scripts/generate-death-cert.py
Output: frontend/public/demo/death-certificate-smith.pdf
"""
...
OUTPUT_PATH = os.path.join(OUTPUT_DIR, "death-certificate-smith.pdf")
...
def generate():
    ...
    c.drawRightString(MARGIN + INNER_W - 12, y - 26, "2026-CH-003891")
    ...
    draw_field(c, MARGIN + col3 + 6, y - FIELD_H, "14. Time of Death", "14:32", col3)
    draw_field(c, MARGIN + 2 * (col3 + 6), y - FIELD_H, "15. County of Death", "Cook", col3)
    y -= ROW_GAP

    draw_field(c, MARGIN, y - FIELD_H, "16. Place of Death", "Hospital — Inpatient", col2)
    draw_field(c, MARGIN + col2 + 6, y - FIELD_H, "17. Facility Name", "Northwestern Memorial Hospital", col2)
    y -= ROW_GAP

    draw_field(c, MARGIN, y - FIELD_H, "18. Facility Address", "251 E. Huron St, Chicago, IL 60611", INNER_W)
    y -= SECT_BEFORE
    ...
    c.setFillColor(DARK)
    c.setFont("Helvetica", 9)
    c.drawString(lx + 13, ly, "Acute myocardial infarction")
    c.setFillColor(MID)
    c.setFont("Helvetica", 7)
    ...
    c.setFillColor(DARK)
    c.setFont("Helvetica", 9)
    c.drawString(lx + 13, ly, "Coronary artery disease")
    c.setFillColor(MID)
    c.setFont("Helvetica", 7)
    c.drawRightString(MARGIN + INNER_W - 8, ly, "Onset to Death: Years")
    ...
    c.setFillColor(DARK)
    c.setFont("Helvetica", 9)
    c.drawString(lx + 13, ly, "Hypertension")
    c.setFillColor(MID)
    c.setFont("Helvetica", 7)
    c.drawRightString(MARGIN + INNER_W - 8, ly, "Onset to Death: Years")
    ...
    # Manner of death row
    draw_field(c, MARGIN, y - FIELD_H, "20. Manner of Death", "Natural", col3)
    draw_field(c, MARGIN + col3 + 6, y - FIELD_H, "21. Autopsy Performed?", "No", col3)
    draw_field(c, MARGIN + 2 * (col3 + 6), y - FIELD_H, "22. Tobacco Contribute?", "Probably", col3)
    y -= SECT_BEFORE
    ...
    draw_field(c, MARGIN, y - FIELD_H, "23. Certifier Type", "Attending Physician", col3)
    draw_field(c, MARGIN + col3 + 6, y - FIELD_H, "24. Certifier Name", "Rachel Nguyen, MD", col3)
    draw_field(c, MARGIN + 2 * (col3 + 6), y - FIELD_H, "25. License No.", "036-078821", col3)
    y -= ROW_GAP

    draw_field(c, MARGIN, y - FIELD_H, "26. Date Certified", "March 3, 2026", col2)
    draw_field(c, MARGIN + col2 + 6, y - FIELD_H, "27. Date Filed", "March 4, 2026", col2)
    y -= ROW_GAP

    draw_field(c, MARGIN, y - FIELD_H, "28. Certifier Address", "251 E. Huron St, Suite 4-200, Chicago, IL 60611", INNER_W)
    y -= FIELD_H + 4
    ...
    c.drawString(MARGIN + 8, sig_y + 3, "R. Nguyen, MD")

    c.setFillColor(LIGHT)
    c.setFont("Helvetica", 6)
    c.drawString(MARGIN, sig_y - 9, "SIGNATURE OF CERTIFIER")
    c.drawString(MARGIN + 210, sig_y - 9, "DATE: March 3, 2026")
    ...
    c.setFillColor(MID)
    c.setFont("Helvetica", 5.5)
    c.drawString(MARGIN, y - 8, "REGISTRAR: Cook County Clerk's Office")
    c.drawString(MARGIN + 220, y - 8, "REGISTERED: March 4, 2026")
    c.drawRightString(MARGIN + INNER_W, y - 8, "STATE FILE NO: 2026-CH-003891")
